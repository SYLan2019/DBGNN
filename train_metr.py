...
def parse_arg():
    ...
    parser.add_argument('--dataset',type=str,default='metr',help='name of the datasets')
    ...
    parser.add_argument('--n_m',type=int,default=52,help='number of mask node during training')
    parser.add_argument('--n_u',type=int,default=103,help='target locations,n_u locations will be deleted from training data')
    ...
    parser.add_argument('--E_maxvalue',type=int,default=80,help='the max value from experience')
    ...
    parser.add_argument('--residual',type=int,default=32,help='the channels of residual')
    parser.add_argument('--dilation', type=int, default=32, help='the channels of dilation')
...
def load_data(dataset):
    ...
    if tod:
        MAX_TOD = 288   #288
        tod = [(i % MAX_TOD) for i in range(X.shape[0])] #list:16992

        tod = [t / MAX_TOD for t in tod]
        tod = np.tile(tod, [1, X.shape[1], 1]).transpose((2, 1, 0))  # [16992,307,1]
        data = np.concatenate((X, tod), axis=-1)
    if dow:
        MAX_TOD = 288   #288
        MAX_DOW = 7
        dow = [((i // MAX_TOD) % MAX_DOW) for i in range(data.shape[0])] #list:16992
    ...
    #choose testnodes
    test_node=np.load('data/metr/testnode.npz')
