...
import torch.nn.functional as F
import logging
# 改自final_metr_addval_res2_attentionfusion_loss.py 将输入经过2个3维图卷积后，残差和结果自注意力融合,对两个分支加入损失限制
#用于pemsbay数据集
def parse_arg():

    parser=argparse.ArgumentParser()
    parser.add_argument('--dataset',type=str,default='pemsbay',help='name of the datasets,select from metr,nrel,ushcn,sedata of pemsbay')
    parser.add_argument('--n_s',type=int,default=165,help='sampled space dimension')  # n_s=n_o+n_m
    ...
    parser.add_argument('--n_m',type=int,default=80,help='number of mask node during training')
    parser.add_argument('--n_u',type=int,default=160,help='target locations,n_u locations will be deleted from training data')
...
def load_data(dataset):
    ...
    test_node=np.load('data/pems/testnode.npz')
